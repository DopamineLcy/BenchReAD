...
def patch_core(
    ...
    def get_patchcore(input_shape, sampler, device):
        ...
        for backbone_name, layers_to_extract_from in zip(
            ...
            if load_backbone:
                if 'flair' in load_backbone:
                    checkpoint = torch.load(load_backbone)
                    key_list = []
                    for key in checkpoint:
                        if "vision_model.model." in key:
                            key_list.append(key)
                    for key in key_list:
                        new_key = key.replace("vision_model.model.", "")
                    ...
                    print(msg)
                elif 'dinov2_meh' in load_backbone:
                    checkpoint = torch.load(load_backbone)
                    ...
                    msg = backbone.load_state_dict(checkpoint, strict=False)
                    print(msg)
                elif 'moco_v2_800ep_pretrain.pth' in load_backbone:
                    checkpoint = torch.load(load_backbone)
...
@click.option("--train_scale", default=1, type=int, show_default=True)
