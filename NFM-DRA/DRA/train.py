...
class Trainer(object):
    ...
    def __init__(self, args):
        ...
        self.train_loader, self.test_loader, self.ref_loader = initDataloader.build(args, **kwargs)
    ...
    def eval(self):
        ...
        with open(self.args.experiment_dir + f'/result_{self.args.test_type}.txt', mode='w', encoding="utf-8") as w:
            for label, score, pred_0, pred_1, pred_2, pred_3 in zip(total_target, total_pred, class_pred[0], class_pred[1], class_pred[2], class_pred[3]):
                w.write(str(label) + '   ' + str(score) + '   ' + str(pred_0) + '   ' + str(pred_1) + '   ' + str(pred_2) + '   ' + str(pred_3) + "\n")
        ...
        plt.savefig(self.args.experiment_dir + f"/vis_{self.args.test_type}.png")
    ...
    def load_weights(self, filename):
        path = os.path.join(self.args.experiment_dir, filename)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_type', type=str, default='valid', help="test type")
    ...
    if args.test_type != "valid":
        trainer.load_weights("best_AUC.pkl")
        auroc, _ = trainer.eval()
    ...
    for epoch in range(0, trainer.args.epochs):
        ...
        if auroc > best_auroc:
            best_auroc = auroc
            trainer.save_weights("best_AUC.pkl")
        print(f"best auroc: {best_auroc}, auroc: {auroc}")
